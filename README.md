### :robot: Hello !

I'm WardDeb.  
Working on bioinformatics, statistics, and things in between.
I'm mainly using Python 🐍 and Rust 🦀, but have experience in R, Julia and SAS as well.

If you want to get in touch:
[✉️ ](mailto:w@rddeboutte.com)

---
### :pager: Recent work

<!--START_SECTION:raa-->
1. 🪢 PR'ed to, 🫸 pushed commit(s) to [deeptools/deepTools](https://github.com/deeptools/deepTools)
2. 🐞 made/updated issue(s) in, 🫸 pushed commit(s) to, 🪢 PR'ed to [linkapy/linkapy](https://github.com/linkapy/linkapy)
3. 🫸 pushed commit(s) to [WardDeb/readme_and_act](https://github.com/WardDeb/readme_and_act)
4. 🫸 pushed commit(s) to [maxplanck-ie/ATACofthesnake](https://github.com/maxplanck-ie/ATACofthesnake)
5. 🫸 pushed commit(s) to [WardDeb/MATES](https://github.com/WardDeb/MATES)
<!--END_SECTION:raa-->

(created with [readme_and_act](https://github.com/marketplace/actions/readme_and_act))